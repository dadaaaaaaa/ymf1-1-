...
double boundaryFunction(double x, double y) {
    return sin(x+y); // Пример функции
}

double rightFunction(double x, double y) {
    //return -12 * (x * x + y * y) + gamma * boundaryFunction(x, y); // Пример функции
    return  3*sin(x + y);
}
...
void writeResultsToFile(const std::string& filename, const std::vector<std::vector<GridNode>>& grid, const std::vector<double>& solutions, const std::vector<double>& b) {
    ...
    for (const auto& row : grid) {
        for (const auto& node : row) {
            int idx = node.index;
            if (b[idx] != 0)
                 difference = std::abs(solutions[idx] - boundaryFunction(node.x, node.y));
            else
                difference = 0;
    ...
    for (int i = 0; i < grid.size(); ++i) {
        for (int j = 0; j < grid[i].size(); ++j) {
            ...
            if (grid[i][j].type == "граничный" && grid[i][j].boundaryType == 1) {
                ...
                double hx = x_nodes[j] - x_nodes[j - 1];
                double hy = y_nodes[i] - y_nodes[i - 1];
                b[idx] = rightFunction(grid[i][j].x, grid[i][j].y);
                A[idx][idx] = _cu(hx, hy);
